...
// Duplicated from lib/posts.ts rather than imported: that module pulls in
// node:fs, which can't ship in the client bundle the admin preview runs in.
const WPM = 200;
...
// A block can still be mid-edit (no photo uploaded yet) when this renders in
// the admin preview — production data from getPosts() never hits this path.
function Photo({ src, alt }: { src: string; alt: string }) {
  if (!src) {
    return (
      <div className="grid h-full w-full place-items-center text-xs uppercase tracking-[0.18em] text-cream/40">
        No photo yet
      </div>
    );
  }
  return <Image src={src} alt={alt} fill sizes="100vw" className={imgClass} style={imgStyle} />;
}
      ...
      <Reveal className="my-14 md:my-20">
        <figure>
          <div className="group relative aspect-[16/9] overflow-hidden rounded-2xl bg-espresso">
            <Photo src={block.src} alt={block.alt} />
          </div>
        ...
        <figure>
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            {block.images.map((im, i) => (
              <div key={i} className="group relative aspect-[4/5] overflow-hidden rounded-xl bg-espresso">
...
// The rendered look of a post — shared by the public /blog/[slug] page and the
// admin editor's live preview, so "what you see" while editing is never a
// second opinion that can drift from what actually publishes.
    ...
    <div className="grain min-h-screen bg-cream">
      <main className="pb-24 pt-28 md:pt-32">
        ...
        <header className="mx-auto max-w-[46rem] px-5">
          <Reveal>
            ...
            <p className="mt-8 text-[0.7rem] uppercase tracking-[0.3em] text-clay">
              {post.category || "Uncategorized"} · {readingTime(post)} min read ·{" "}
              {dateFmt.format(post.date ? new Date(post.date) : new Date())}
            </p>
        ...
        <Reveal className="mx-auto mt-12 max-w-[1400px] px-4 md:mt-16 md:px-8">
          <div className="group relative aspect-[16/9] overflow-hidden rounded-2xl bg-espresso">
            <Photo src={post.cover} alt={post.coverAlt} />
          </div>
        </Reveal>
